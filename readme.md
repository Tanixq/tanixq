# 👋 Hi there, I'm Tanishq Patel!


### 💻 Java Developer | Backend Enthusiast | Problem Solver | Node.JS Developer

---

### 🚀 About Me
- 🏢 **4 years of experience at Persistent Systems** working on high-impact Java projects and backend development.
- 🔥 Passionate about building **robust backend systems**.
- 💡 Skilled in **Java**, **Spring Boot**, **Hibernate**, **SQL**, and **REST APIs**.
- 📚 Always learning new technologies to build better and scalable software.
- 🛠️ Currently exploring **Microservices**, **Cloud (AWS)**, and **System Design**.

---

### 🛠️ Tech Stack
- **Languages:** Java, JavaScript, Node.JS, Terraform
- **Frameworks:** Spring Boot, Hibernate, JUnit, Express.JS
- **Databases:** MySQL, PostgreSQL, MongoDB
- **Tools:** Git, Maven, Docker, Postman
- **Cloud:** Basics of AWS (EC2, S3)

---

### 🚀 My Skillset
#### Java Expertise
![Java](https://img.shields.io/badge/Java-007396?style=for-the-badge&logo=java&logoColor=white)

#### Spring Boot
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white)

#### SQL
![SQL](https://img.shields.io/badge/SQL-F4B400?style=for-the-badge&logo=database&logoColor=white)

---

### ⚡ Fun Fact:
> "Code is like humor. When you have to explain it, it’s bad."

![Bounce Effect](https://media.giphy.com/media/JIX9t2j0ZTN9S/giphy.gif)

---

### 📫 Connect With Me

- LinkedIn: https://linkedin.com/in/tanixq
- Email: tanishqpatel002@gmail.com